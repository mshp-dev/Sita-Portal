            ...
            try:
                ...
                if invoice_type.code == 'INVDIR':
                    ...
                    if permissions.filter(permission__in=[1, 2, 32, 4]).exists():
                        ...
                        invoice.save()
                        logger.info(f'invoice with serial number {invoice.serial_number} generated by {isc_user.user.username}.', request)
                        response = {
                            'result': 'success',
                            'invoice_id': invoice.id
                        }
        ...
        if request.method == 'POST':
            ...
            if invoice_type.code == 'INVDIR':
                ...
                if isc_user.role.code == 'ADMIN':
                    ...
                    logger.info(f'invoice with serial number {invoice.serial_number} confirmed by {isc_user.user.username}.', request)
                else:
                    ...
                    if invoice_type.code == 'INVUNLS':
                        ...
                        for p in perms_list:
                            if Permission.objects.filter(pk=p).exists():
                                ...
                                logger.warn(f'permission with id {p} does not exists.', request)
    ...
    if request.is_ajax():
        if request.method == 'POST':
            ...
            if isc_user.role.code == 'ADMIN':
                ...
                logger.info(f'invoice with serial number {invoice.serial_number} rejected by {isc_user.user.username}.', request)
            else:
                logger.critical(f'unauthorized trying reject of invoice with serial number {invoice.serial_number} by {isc_user.user.username}.', request)
                response = {
                    'result': 'error',
                    'message': 'شما مجاز به انجام این کار نیستید'
                }
    ...
    if request.is_ajax():
        if request.method == 'POST':
            ...
            if isc_user.role.code == 'ADMIN':
                ...
                logger.info(f'status of invoice with serial number {invoice.serial_number} updated to undefined by {isc_user.user.username}.', request)
            else:
                logger.critical(f'unauthorized trying update of invoice with serial number {invoice.serial_number} by {isc_user.user.username}.', request)
                response = {
                    'result': 'error',
                    'message': 'شما مجاز به انجام این کار نیستید'
                }
    ...
    if request.is_ajax():
        if request.method == 'POST':
            ...
            if invoice.created_by == isc_user or isc_user.role.code == 'ADMIN':
                ...
                invoice.delete()
                logger.info(f'invoice with serial number {invoice.serial_number} deleted by {isc_user.user.username}.', request)
            else:
...
def invoice_details_view(request, iid, *args, **kwargs):
    ...
    if request.is_ajax():
        if request.method == 'POST':
            ...
            logger.info(f'invoice with serial number {invoice.serial_number} downloaded by {isc_user.user.username}.', request)
            return response
