...
class BusinessCode(BaseCoding):
    code             = models.CharField(max_length=50, blank=False, unique=True, default='...')
...
class IscUser(models.Model):
    ...
    def get_isc_user_owned_businesses(self):
        ...
        if self.role.code == 'ADMIN':
            opr_buss = BusinessCode.objects.all().exclude(code__startswith='SETAD_', code=models.F('description')).order_by('description')
        else:
...
class MftUser(models.Model):
    ...
    password_expiration_interval = models.IntegerField(blank=True, default=-1)
    ...
    def set_all_used_business(self):
        bus_list = BusinessCode.objects.all().exclude(code__startswith='SETAD_', code=models.F('description')).exclude(code__in=self.business.all().values('code')).values('code')
